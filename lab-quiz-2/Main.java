    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < TOTAL_STUDENTS; i++) {
            ...
            if (i < TOTAL_STUDENTS / 2) {
                list2[i] = new EveningBatch();
                list2[i].challanPaid = students[i].getChallan();
                list2[i].marks = students[i].marks;
                list2[i].batch = "Morning";
            } else {
                list1[i - half] = new MorningBatch();
                list1[i - half].marks = students[i].marks;
                list1[i - half].batch = "Evening";
                list1[i - half].challanPaid = students[i].getChallan();
            }
        }
